...
echo "$($PSQL "SELECT AVG(x.sum) FROM (SELECT game_id, sum(winner_goals + opponent_goals) AS sum FROM games GROUP BY game_id)x")"

echo -e "\nMost goals scored in a single game by one team:"
echo "$($PSQL "SELECT MAX(x.goals) FROM (SELECT winner_goals AS goals FROM games UNION SELECT opponent_goals AS goals FROM games)x")"
